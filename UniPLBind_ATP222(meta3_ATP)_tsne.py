...
class FeatureExtractor(nn.Module):
    ...
    def forward(self, prot_input, seqlengths):
        ...
        # cache features for t-SNE visualization
        if self.enable_cache:
            self.cache = {
                'plm': prot_input.detach(),
                'mscnn': mscnn.detach(),
                'attention': s.detach(),
                'final': final_feat.detach()
            }
...
def test_tsne(
    ...
    """
    Generate t-SNE visualization for residue-level representations.
    ...
    Note:
        ...
        It does not affect model training or testing metrics.
    """
    ...
    # 模型输入维度必须和训练时保持一致
    model = Module(1536 + 1024, False).to(device)
    model.load_state_dict(torch.load(modelstoreapl, map_location=device))
    model.eval()

    model.feature_extractor.enable_cache = True
    ...
    with torch.no_grad():
        for prot_xs, data_ys, taskids, lengths in test_loader:
            ...
            # forward
            outputs = model(prot_xs, lengths_cpu)

            # get cached features from current model
            if not hasattr(model.feature_extractor, 'cache'):
                raise RuntimeError(
                    "Cannot find model.feature_extractor.cache. "
                    "Please add self.cache in FeatureExtractor.forward()."
                )

            cache = model.feature_extractor.cache

            plm_batch = cache['plm'].detach().cpu().numpy()
            mscnn_batch = cache['mscnn'].detach().cpu().numpy()
            attention_batch = cache['attention'].detach().cpu().numpy()
            final_batch = cache['final'].detach().cpu().numpy()

            labels_batch = data_ys.cpu().numpy()
            lens = lengths_cpu.numpy().tolist()

            # remove padded residues
            for b, L in enumerate(lens):
                all_plm_feats.append(plm_batch[b, :L, :])
                all_mscnn_feats.append(mscnn_batch[b, :L, :])
                all_attention_feats.append(attention_batch[b, :L, :])
    ...
    def _stack_and_sample(feat_list, label_list, max_points=5000, seed=42):
        ...
        y = np.concatenate(label_list).astype(int)
    ...
    def _tsne_plot(X, y, title, outfile, perplexity=30, lr=200, seed=42):
        ...
        png_path = os.path.join(tsne_outdir, f"{stem}.png")
        eps_path = os.path.join(tsne_outdir, f"{stem}.eps")
    ...
    X_mscnn, y_mscnn = _stack_and_sample(all_mscnn_feats, all_labels, tsne_max_points)
    X_attention, y_attention = _stack_and_sample(all_attention_feats, all_labels, tsne_max_points)
    ...
    if plot_plm:
        _tsne_plot(
            X_plm,
            y_plm,
            f't-SNE of Raw PLM Embeddings ({prefix})',
            f'{prefix}_tsne_plm.png'
        )
    ...
    if plot_mscnn:
        _tsne_plot(
            X_mscnn,
            y_mscnn,
            f't-SNE of Multi-scale CNN Features ({prefix})',
            f'{prefix}_tsne_mscnn.png'
        )
    ...
    if plot_attention:
        _tsne_plot(
            X_attention,
            y_attention,
            f't-SNE of Attention-refined Features ({prefix})',
            f'{prefix}_tsne_attention.png'
        )
    ...
    if plot_final:
        _tsne_plot(
            X_final,
            y_final,
            f't-SNE of Final UniPLBind Features ({prefix})',
            f'{prefix}_tsne_final.png'
        )
    ...
    for i in range(circle):
        ...
        if i == 0:
            test_tsne(
                storeapl,
                tsne_max_points=5000,
                tsne_outdir=f'RGMTL/tsne_{storename}_{i}_{timestamp}',
                prefix=f'{storename}_{i}',
                plot_plm=True,
                plot_mscnn=True,

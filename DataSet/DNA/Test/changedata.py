def process_rbp_data(input_file, output_file):
    with open(input_file, 'r') as infile, open(output_file, 'w') as outfile:
        while True:
            ...
            # 写入新的两行格式
            outfile.write(f"{protein_id}_DNA\t{sequence}\n")
            outfile.write(f"{protein_id}_DNA\t{binding}\n")


# 使用示例
input_filename = "DNA-129_Test.txt"  # 替换为你的输入文件名
output_filename = "DNA_Test.txt"  # 输出文件名

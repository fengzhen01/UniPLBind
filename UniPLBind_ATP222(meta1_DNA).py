...
def read_data_file_trip(istraining):
    ...
    if istraining:
        # file_path = 'D:/fengzhen/1NucGMTL-main/DataSet/SMB/SMB_Train.txt'
        file_path = 'D:/fengzhen/1NucGMTL-main/DataSet/DNA/Train/DNA_Train.txt'
    else:
        file_path = 'D:/fengzhen/1NucGMTL-main/DataSet/DNA/Test/DNA_Test.txt'
...
class BioinformaticsDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        df0 = pd.read_csv('D:/fengzhen/1embedding/ProstT5_embedding_573+129/' + filename + '.data', header=None)
        prot0 = df0.values.astype(float).tolist()
        prot0 = torch.tensor(prot0)

        df1 = pd.read_csv('D:/fengzhen/1embedding/Ankh_embedding_573+129/' + filename + '.data', header=None)
...
def train_vnet(itrainfile, modelstoreapl, valfile=None):
    ...
    model = Module(1024 + 1536, True).to(device)
...
def test(modelstoreapl, testfile=None):
    ...
    model = Module(1024 + 1536, False).to(device)
...
#         sensitivity, specificity, acc, precision, mcc, auc, aupr_1 = printresult('DNA', labels_actual, predicted_probs, labels_predicted)
#         totalkv['DNA'].append([sensitivity, specificity, acc, precision, mcc, auc, aupr_1])
#
#     with open(f'RGMTL/Result_{storename}_{a}.txt', 'w') as f:
#         np.savetxt(f, totalkv['DNA'], delimiter=',', fmt='%s', footer='DNA Results')
#         m = np.mean(totalkv['DNA'], axis=0)
...
if __name__ == "__main__":
    ...
    for i in range(circle):
        ...
        sensitivity, specificity, acc, precision, mcc, auc, aupr_1 = printresult(
            'DNA', labels_actual, predicted_probs, labels_predicted
        )
        totalkv['DNA'].append([sensitivity, specificity, acc, precision, mcc, auc, aupr_1])

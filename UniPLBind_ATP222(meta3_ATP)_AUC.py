...
class BioinformaticsDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        df0 = pd.read_csv('E:/fengzhen/embedding_ATP1/ESM1b_embedding_387_41/' + filename + '.data', header=None)
        prot0 = df0.values.astype(float).tolist()
        prot0 = torch.tensor(prot0)

        df1 = pd.read_csv('E:/fengzhen/embedding_ATP1/ProtBert_embedding_387_41/' + filename + '.data', header=None)
...
def train_vnet(itrainfile, modelstoreapl, valfile=None):
    ...
    model = Module(1280 + 1024, True).to(device)
...
def test(modelstoreapl, testfile=None):
    model = Module(1280 + 1024, False).to(device)
    ...
    with torch.no_grad():
        for prot_xs, data_ys, taskids, lengths in test_loader:
            ...
            # outputs: [batch_size, max_seq_len, 2]
            outputs = model(prot_xs, lengths)

            # probs: [batch_size, max_seq_len, 2]
            probs = F.softmax(outputs, dim=-1)
            preds = torch.argmax(probs, dim=-1)

            # 只保留每个蛋白真实长度内的残基，排除 padding
            for batch_idx, seq_len in enumerate(lengths.tolist()):
                valid_probs = probs[batch_idx, :seq_len, 1]
                valid_preds = preds[batch_idx, :seq_len]
                valid_labels = data_ys[batch_idx, :seq_len]

                predicted_probs.extend(valid_probs.detach().cpu().tolist())
                labels_predicted.extend(valid_preds.detach().cpu().tolist())
                labels_actual.extend(valid_labels.detach().cpu().tolist())
